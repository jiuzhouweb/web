                ...
                <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                    <el-tab-pane :label="item.title" :name="item.name" v-for="(item,index) in tableTabs" :key="index">
                        <el-table v-loading="loading" :data="item.tableData" border style="width: 100%">
                            ...
                            <el-table-column v-if="item.name==2||item.name==4||item.name==5" :prop="item.name==2?'monIncomeAmount':'incomeAmount'" label="本期收入">
                            </el-table-column>
                            <el-table-column v-if="item.name==2||item.name==4||item.name==5" :prop="item.name==2?'monTaxFreeIncome':'taxFreeIncome'" label="本期免税收入" width="110">
                            </el-table-column>
                        ...
                        <el-pagination style="margin-top:10px;" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 20, 30, 40,50]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
                        </el-pagination>
                    </el-tab-pane>
                </el-tabs>
            </div>
